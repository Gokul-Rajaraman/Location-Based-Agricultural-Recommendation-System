...
df_final=pd.read_csv(r'Dataset for crop recommendation.csv')
import pickle
with open('rf_model_crop.pkl', 'rb') as rf_model_file:
    rf = pickle.load(rf_model_file)
with open('et_model_crop.pkl', 'rb') as et_model_file:
    et = pickle.load(et_model_file)
with open('bagging_model_crop.pkl', 'rb') as bagging_model_file:
    bagging = pickle.load(bagging_model_file)
from geopy.distance import great_circle
def getpredictors(latitude,longitude):
    ...
    crop_list=['BARLEY', 'GROUNDNUT', 'MAIZE', 'RICE', 'SUGARCANE', 'WHEAT']
    pred_rf=rf.predict(X_feed)
    pred_et=et.predict(X_feed)
    pred_bg=bagging.predict(X_feed)
